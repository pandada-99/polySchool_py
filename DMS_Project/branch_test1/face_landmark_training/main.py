...
def make_model(inSahpe, outShape=24):
    ...
    # Layer 3
    model.add(Conv2D(16, (5, 5), activation='tanh', padding='same'))  # Conv2d(filter, kernel_size 부터 시작한다)
...
def read_text_to_np():
    file = open("../face_landmark_train_set/Train_daytime/", "r")
...
def make_X_Y(path=None):
    ...
    while True:
        ...
        pos_array.append(np.array(list(map(int, line.strip().split("\t")[1:]))))
        # print(cnt)
...
Xtrain, Ytrain = make_X_Y("../face_landmark_train_set/Train_daytime/")
...
Xtest, Ytest = make_X_Y("../face_landmark_train_set/test/8_test_sj/")
print(type(Xtest))
print(type(Ytest))
MY_EPOCH = 20
MY_BATCHSIZE = 20
