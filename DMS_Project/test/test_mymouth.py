...
import cv2, dlib # cv2 => 사람 얼굴 탐지 / dlib => 사람 얼굴 인식하는 모델
import numpy as np
import time
...
if video_capture.isOpened():
    ...
    while True:
        ...
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) # 흑백으로 바꾸기
        clahe = cv2.createCLAHE(clipLimit=2.0, # 좀 더 선명하게 만들기
                                tileGridSize=(8, 8))
        clahe_image = clahe.apply(gray)
        detection = face_detector(clahe_image, 0) # 얼굴 찾은 걸 저장

        if detection:  # 얼굴을 detection 했을때
            for d in detection:
                shape = shape_predictor(clahe_image, d) # 얼굴 찾은 네모위치 안에 점 68개 찾아주기
                ...
                if d2detection:
                    for d2 in d2detection:
                        ...
                        # d2landmarks => 68개 점이 들어가있음 / 이걸 함수 인자로 받아서 함수 안에서 원하는 점을 가지고 고개를 숙였다 아니다를 리턴하는 함수 만들기

                        # # 함수 호출
                        # count_head = deque(maxlen=1)
                        # count_head.append(head(d2landmarks))
                        # if (sum(count_head)) > 0:
                        #     cv2.putText(dst_frame, "head!!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, GREEN, 2)
                        #     count_sleep.append(1)
                        #     if (sum(count_sleep)) > 20:
                        #         cv2.putText(dst_frame, "sleep_head!!", (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, GREEN, 2)
                        # else:
                        #     if sum(count_sleep) > 0:
                        #         count_sleep.pop()

                        # 함수 호출
                        if (head(d2landmarks) == 1):
                            ...
                            if count_head >= min_count:
                                count_head -= 1
                        if count_head > 25:
                            cv2.putText(dst_frame, "sleep_head!!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, SKYBLUE, 2)
                        # print(count_head)

                        # 함수 호출
                        if (mouth(d2landmarks) == 1):
                            ...
                            if count_mouth >= min_count:
                                count_mouth -= 1
                        if count_mouth > 30:
                            cv2.putText(dst_frame, "sleep_mouth!!", (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, SKYBLUE, 2)
                        print(count_mouth)
                        ...
                        # 졸음의 정도를  driving_state_step 수치를 조정해야한다 [0]활성화 임계점 [1]자는 임계점
        ...
        cv2.putText(frame, f"FPS:{int(1. / (time.time() - start_t))}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, RED, 2)
